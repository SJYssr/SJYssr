...
# 生成进度条
def generate_progress_bar(percentage, length=50):
...
for i, day in enumerate(weekday_names):
    ...
    weekday_section += f"{day:<10} {progress_bar} {count} ({percentage:.1f}%)\n"
weekday_section += '</pre>'
...
for period in ['morning', 'afternoon', 'evening', 'night']:
    ...
    progress_bar = generate_progress_bar(percentage)
    time_period_section += f"{time_period_names[period]:<20} {progress_bar} {count} ({percentage:.1f}%)\n"
time_period_section += '</pre>'
